...
from core.utils import is_correct, init_answer, read_json, read_prompts, get_avatar, get_color, get_role
...
class System(ABC):
    ...
    def __init__(self, task: str, config_path: str, leak: bool = False, web_demo: bool = False, dataset: Optional[str] = None, *args, **kwargs) -> None:
        ...
        self.config = read_json(config_path)    # 'config/systems/collaboration/all_agents.json'
        ...
        self.kwargs = kwargs
        self.mode_input = False
        self.init(*args, **kwargs)
        self.reset(clear=True)
    
    def log(self, message: str, agent: Optional[Agent] = None, logging: bool = True, omit: bool = False) -> None:
        """Log the message.
        ...
        if self.web_demo:
            ...
            final_message = f':{get_color(role)}[**{get_role(role)}**]: {message}'

            if omit:
                messages = final_message.split('\n')
                messages = [messages[0]] + [' '.join(messages[1].split()[:12])]
                final_message = '\n'.join(messages) + ' ...'
